...
async function getTokenEncryptionKey() {
  const secret = requireEnv('HUBSPOT_TOKEN_ENCRYPTION_KEY');
...
export function getHubSpotScopes() {
  return ['crm.objects.contacts.read', 'crm.objects.contacts.write', 'crm.objects.notes.write'];
}
...
export async function exchangeCodeForTokens(code: string) {
  ...
  if (!response.ok) throw new Error(`HubSpot token exchange failed: ${await response.text()}`);
...
export async function refreshHubSpotToken(connectionId: string) {
  ...
  if (!response.ok) throw new Error(`HubSpot token refresh failed: ${await response.text()}`);
  const refreshed = await response.json();
  const nextExpiresAt = new Date(Date.now() + Number(refreshed.expires_in ?? 1800) * 1000).toISOString();
  ...
  await supabase
    ...
    .update({ expires_at: nextExpiresAt, updated_at: new Date().toISOString(), status: 'active' })
    .eq('id', connectionId);
...
export async function hubspotRequest(accessToken: string, path: string, init: RequestInit = {}) {
...
export async function getHubSpotIdentity(accessToken: string) {
  ...
  if (!response.ok) throw new Error(`HubSpot identity lookup failed: ${await response.text()}`);
  return await response.json();
}
...
export function hubspotContactToLeadPayload(contact: HubSpotContact, organizationId: string, userId: string) {
  ...
  return {
    ...
    location: properties[CUSTOM_CONTACT_PROPERTIES.location] ?? [properties.city, properties.state, properties.country].filter(Boolean).join(', '),
    timezone: '',
    tags: ['HubSpot'],
    last_contact: '',
    crm_source: 'hubspot',
...
export async function fetchHubSpotContactPropertyNames(accessToken: string) {
  ...
  return new Set<string>((body.results ?? []).map((property: { name: string }) => property.name));
}

export function leadToHubSpotProperties(lead: Record<string, unknown>, availableProperties: Set<string>) {
  ...
  if (availableProperties.has(CUSTOM_CONTACT_PROPERTIES.location)) {
    properties[CUSTOM_CONTACT_PROPERTIES.location] = String(lead.location ?? '');
  }
...
export async function fetchAllHubSpotContacts(accessToken: string, availableProperties = new Set<string>()) {
  const contacts: HubSpotContact[] = [];
  let after: string | undefined;
  const properties = ['email', 'firstname', 'lastname', 'phone', 'company', 'jobtitle', 'city', 'state', 'country', 'lastmodifieddate'];
  ...
  do {
    ...
    const body = await hubspotRequest(accessToken, `/crm/v3/objects/contacts?${params.toString()}`);
