...
export function LeadSearchPanel({ activityCollapsed, onToggleActivity }: LeadSearchPanelProps) {
  ...
  return (
    <div className="relative w-12 flex-shrink-0 border-r flex flex-col items-center gap-2 py-4" style={{ borderColor: 'var(--dash-border)', background: 'var(--dash-topbar)' }}>
      ...
      <button type="button" title="Back to the top" disabled={leads.length === 0 || currentIndex === 0} onClick={jumpToFirstLead}
        ...
        <span className="pointer-events-none absolute left-12 bottom-5 whitespace-nowrap rounded-md border px-2 py-1 text-xs font-semibold opacity-0 shadow-2xl transition-opacity group-hover:opacity-100" style={{ background: 'var(--dash-elevated)', borderColor: 'var(--dash-border)', color: 'var(--dash-text)' }}>
          Back to the top
        </span>
      </button>
      <AnimatePresence>
        {showLeadSearch && (
          <motion.div initial={{ opacity: 0, x: -10, scale: 0.96 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: -10, scale: 0.96 }}
            transition={{ duration: 0.15 }} className="absolute left-12 top-16 z-30 w-72 rounded-xl border p-3 shadow-2xl" style={{ background: 'var(--dash-elevated)', borderColor: 'var(--dash-border)' }}>
            <div className="flex items-center gap-2">
              ...
              <input autoFocus value={leadSearchQuery}
                ...
                placeholder="Search leads" className="min-w-0 flex-1 bg-transparent text-sm focus:outline-none" style={{ color: 'var(--dash-text)' }} />
            </div>
            <div className="mt-3 flex items-center justify-between">
              <span className="text-xs" style={{ color: 'var(--dash-muted)' }}>{leadSearchQuery ? `${leadSearchMatchCount} match${leadSearchMatchCount === 1 ? '' : 'es'}` : 'Type to search'}</span>
